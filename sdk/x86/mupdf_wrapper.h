...
// PrintOptions 完整定义（同步自 print_engine.h）
 ...
 * @param rule 位置规则（NULL=使用默认规则，左上角，margin_y=10）
 *        - type=1 左上, type=2 右上, type=3 左下, type=4 右下
 *        - margin_y: 正值向下移动，负值向上移动
 * @param width 固定宽度（0=自动）
 * @param height 固定高度（0=自动）
 ...
 * @param x X坐标（pt）
 * @param y Y坐标（pt）
 * @param page_suffix_separator 页码分隔符（如 " - "），传入 NULL 则不添加页码
 * @return 成功返回 0
 * @note 文字会自动拼接为: "text + page_suffix_separator + 当前页/总页数"
 *       例如: text="机密文件", page_suffix_separator=" - " → "机密文件 - 1/5"
 */
MUPDF_API int mupdf_add_text_with_page_number(
    MupdfContext* ctx,
    MupdfDocument* doc,
    int page_num,
    const char* text,
    float font_size,
    float x, float y,
    const char* page_suffix_separator
);
 ...
 *        - add_page_number: 0=不添加页码, 1=添加 " - 页/总页" 后缀
 * @return 成功返回添加的页面数
 */
